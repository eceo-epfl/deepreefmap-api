...
async def seed_db(postgres_url):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print(f"Usage: python {sys.argv[0]} <postgres_url>")
        print("eg. postgresql+asyncpg://user:password@localhost:5432/postgres")
        sys.exit(1)

    postgres_url = sys.argv[1]

    loop = asyncio.get_event_loop()
    loop.run_until_complete(seed_db(postgres_url))
